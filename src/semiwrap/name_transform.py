...
class NameTransformConfig:
    ...
    #: Case-sensitive acronyms used by built-in case transforms when splitting words.
    acronyms: typing.Optional[typing.List[str]] = None
...
def _normalize_acronyms(acronyms: typing.Optional[Acronyms]) -> typing.Tuple[str, ...]:
    if not acronyms:
        return ()
    return tuple(sorted({a for a in acronyms if a}, key=len, reverse=True))


def _split_words_no_acronyms(name: str) -> typing.List[str]:
...
def _split_part_with_acronyms(
    ...
    while pos < len(part):
        ...
        if segment_start < pos:
            words.extend(_split_words_no_acronyms(part[segment_start:pos]))
        words.append(match)
        pos += len(match)
        segment_start = pos

    if segment_start < len(part):
        words.extend(_split_words_no_acronyms(part[segment_start:]))

    return words


def _split_words(
    name: str, acronyms: typing.Optional[Acronyms] = None
) -> typing.List[str]:
    normalized_acronyms = _normalize_acronyms(acronyms)
    if not normalized_acronyms:
        return _split_words_no_acronyms(name)

    words: typing.List[str] = []
    for part in name.replace("-", "_").split("_"):
        if not part:
            continue
        words.extend(_split_part_with_acronyms(part, normalized_acronyms))
    return words or [name]
...
def merge_name_transform_configs(
    ...
    return NameTransformConfig(
        ...
        acronyms=(
            higher_cfg.acronyms
            if higher_cfg.acronyms is not None
            else lower_cfg.acronyms
        ),
    )
...
def resolve_name_transforms(
    ...
    selected_acronyms = acronyms if acronyms is not None else cfg.acronyms
    ...
    return NameTransforms(
        function=resolve_name_transform(function_spec, selected_acronyms),
        method=resolve_name_transform(method_spec, selected_acronyms),
        attribute=resolve_name_transform(attribute_spec, selected_acronyms),
        enum_value=resolve_name_transform(enum_value_spec, selected_acronyms),
        parameter=resolve_name_transform(parameter_spec, selected_acronyms),
    )
...
def name_transform_config_to_args(spec: NameTransformSpec) -> typing.List[str]:
    ...
    if cfg.acronyms is not None:
        for acronym in cfg.acronyms:
            args.extend(["--name-transform-acronym", acronym])
    return args
...
def resolve_name_transform(
    spec: str, acronyms: typing.Optional[Acronyms] = None
) -> NameTransform:
    transform = _resolve_name_transform(spec)
    normalized_acronyms = _normalize_acronyms(acronyms)
    if not normalized_acronyms or spec not in _BUILTINS:
        return transform

    def wrapper(name: str, kind: NameKind) -> str:
        result = transform(name, kind, normalized_acronyms)  # type: ignore[misc]
