...
def _render_cls_trampoline(
    ...
    * Allow python programs access to protected members
    ...
    nb_slots = len(trampoline.protected_constructors) + len(trampoline.virtual_methods)
    ...
    if cls.bases:
        ...
        r.write_trim(
            ...
            struct PyTrampoline_{ cls.cpp_name } : PyTrampolineBase_{ cls.cpp_name }<PyTrampolineBase{ precomma(template_argument_list) }, PyTrampolineCfg> {{
              using NBTmplBase = PyTrampolineBase_{ cls.cpp_name }<PyTrampolineBase{ precomma(template_argument_list) }, PyTrampolineCfg>;
              NB_TRAMPOLINE(NBTmplBase, {nb_slots});
        """
        )
        ...
        r.write_trim(
            ...
            struct PyTrampoline_{ cls.cpp_name } : PyTrampolineBase {{
              NB_TRAMPOLINE(PyTrampolineBase, {nb_slots});             
        """
        )
        ...
        with r.indent():
            if fn.trampoline_cpp_code:
                ...
                if fn.is_pure_virtual and fn.virtual_xform:
                    r.write_trim(
                        f"""
                        SEMIWRAP_OVERRIDE_XFORM_PURE("{fn.py_name}", {fn.cpp_name}, {all_names});
                        """
                    )
                elif fn.is_pure_virtual:
                    r.write_trim(
                        f"""
                        using nb_ret_type = {fn.cpp_return_type};
                    ...
                    r.write_trim(
                        ...
                        SEMIWRAP_OVERRIDE_XFORM("{fn.py_name}", {fn.cpp_name}, {all_names})
...
def _render_cls_template_impl(
    ...
    with r.indent():
        ...
        r.writeln(
            f"bind_{cls.full_cpp_name_identifier}(nb::module_ &m, const char * clsName) :"
        )
            ...
            r.write_trim(
                ...
                if (add_doc) {{
                    ...
                    if (doc.is_none()) {{
                        ...
                        {cls.var_name}.doc() = nb::str(doc) + nb::str(add_doc);
                    }}
                }}
            """
            )
