...
def mkdoc(pre: str, doc: Documentation) -> str:
    if doc:
        if len(doc) == 1:
            return f"{pre}{doc[0]}"
...
def _genmethod(
    ...
    if fn.operator:
        ...
        if fn.cpp_code:
            r.writeln(f'{varname}.def("__init__", ({fn.cpp_code})')
        elif fn.genlambda:
            ...
            r.writeln(
                f"{varname}.def(nb::init<{', '.join(param.full_cpp_type for param in arg_params)}>()"
            )
    else:
    ...
    for nurse, patient in fn.keepalives:
        other_params.append(f"nb::keep_alive<{nurse}, {patient}>()")
...
def _genprop(r: RenderBuffer, varname: str, qualname: str, prop: PropContext):
    ...
    if prop.array_size:
        ...
        propdef = ".def_prop_ro" if prop.readonly else ".def_prop_rw"
        ...
        propdef = ".def_ro" if prop.readonly else ".def_rw"
        if prop.static:
...
def enum_decl(r: RenderBuffer, enum: EnumContext, varname: str):
    r.writeln(f"nb::enum_<{ enum.full_cpp_name }> {varname};")
...
def enum_init_args(scope: str, enum: EnumContext):
    ...
    if enum.arithmetic:
        params.append("nb::is_arithmetic()")
...
def cls_decl(r: RenderBuffer, cls: ClassContext):
    if cls.trampoline:
        ...
        r.write_trim(
            f"""
            struct {tctx.var} : {tctx.full_cpp_name} {{
    ...
    # if cls.nodelete:
    #     class_params.append(
    #         f"std::unique_ptr<typename {cls.full_cpp_name}, swgen::nodelete>"
    #     )
    # else:
    ...
    r.writeln(f"nb::class_<{', '.join(class_params)}> {cls.var_name};")
...
def cls_def(r: RenderBuffer, cls: ClassContext, varname: str):
    ...
    if cls.add_default_constructor:
        r.writeln(f"{varname}.def(nb::init<>(), release_gil());")
