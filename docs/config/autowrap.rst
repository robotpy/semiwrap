...
Arguments that have Python defaults can be generated as keyword-only arguments by
setting ``default_args_as_kw_only``. It defaults to ``false`` and can be enabled
for the entire YAML file:
...
Or configured for a function, method, or individual overload:
...
.. code-block:: yaml
   ...
   functions:
     calculate:
       default_args_as_kw_only: true
     legacy_function:
       # Explicit settings override the file default.
       default_args_as_kw_only: false
     overloaded_function:
       default_args_as_kw_only: true
       overloads:
         int, int:
...
Precedence is overload, then function or method, then the file default. An
explicit ``false`` overrides an inherited ``true``.

Semiwrap emits ``py::kw_only()`` before the first exposed argument that retains
...
You may also specify a python function to perform custom transform logic.
...
When a YAML file specifies ``known_words: []``, it uses no known words for
that header.
