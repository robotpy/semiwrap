...
def test_autowrap_yaml_accepts_top_level_known_words(tmp_path):
    yml = tmp_path / "x.yml"
    yml.write_text("known_words: [KiB, mDNS]\n")
    cfg = AutowrapConfigYaml.from_file(yml)
    assert cfg.known_words == ["KiB", "mDNS"]


def test_pyproject_name_transform_accepts_known_words():
    tool = SemiwrapToolConfig(
        name_transform=NameTransformConfig(default="snake_case", known_words=["KiB"])
    )
    ext = ExtensionModuleConfig(
        name_transform=NameTransformConfig(method="camelCase", known_words=["mDNS"])
    )
    assert tool.name_transform == NameTransformConfig(
        default="snake_case", known_words=["KiB"]
    )
    assert ext.name_transform == NameTransformConfig(
        method="camelCase", known_words=["mDNS"]
    )


def test_name_transform_config_expands_known_words_to_repeated_command_line_flags():
    cfg = NameTransformConfig(default="snake_case", known_words=["KiB", "mDNS"])
    assert name_transform_config_to_args(cfg) == [
        "--name-transform-default",
        "snake_case",
...
def test_header2dat_uses_cli_known_words_when_yaml_omits_known_words(tmp_path):
    header = tmp_path / "x.h"
    yml = tmp_path / "x.yml"
    dat = tmp_path / "x.dat"
    dep = tmp_path / "x.d"
    ...
    generate_wrapper(
        ...
        name_transform_known_words=["KiB"],
    )
...
def test_header2dat_yaml_known_words_override_cli_known_words(tmp_path):
    header = tmp_path / "x.h"
    yml = tmp_path / "x.yml"
    dat = tmp_path / "x.dat"
    dep = tmp_path / "x.d"
